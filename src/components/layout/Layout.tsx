...
export const Layout = ({ children, openModal }: LayoutProps) => {
  ...
  return (
    <div className="min-h-screen bg-[#050505] text-white selection:bg-blue-500/30 selection:text-blue-100">
      <header className="fixed top-0 left-0 right-0 z-50 px-2 sm:px-4 lg:px-6 pt-2.5 sm:pt-3">
        <div
        ...
        >
          <div className="h-[74px] sm:h-[84px] lg:h-[94px] px-3 sm:px-6 lg:px-8 flex items-center justify-between gap-3">
            <Link to="/" className="group inline-flex items-center gap-2 sm:gap-4 shrink-0 min-w-0">
            ...
            <button
              ...
              className="md:hidden p-2.5 rounded-md border border-white/10 bg-white/[0.03] text-white/90 hover:text-white hover:border-white/25 transition-colors"
            >
        ...
        <AnimatePresence>
          {isMenuOpen && (
            <motion.div
              ...
              exit={{ opacity: 0, y: -8 }}
              className="md:hidden mt-2 mx-auto max-w-7xl rounded-lg border border-white/15 bg-[#070a10]/95 backdrop-blur-xl shadow-[0_14px_28px_rgba(0,0,0,0.45)] max-h-[calc(100vh-112px)] overflow-y-auto"
            >
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="grid grid-cols-1 md:grid-cols-4 gap-10 md:gap-12 mb-14 sm:mb-16 lg:mb-20">
            <div className="col-span-1 md:col-span-2">
              ...
              <div className="flex gap-4">
                <a href={BUSINESS_INFO.instagramUrl} target="_blank" rel="noopener noreferrer" className="w-11 h-11 sm:w-12 sm:h-12 border border-white/10 flex items-center justify-center hover:bg-blue-600 hover:border-blue-600 transition-all group">
                  <Instagram className="w-4 h-4 sm:w-5 sm:h-5 text-gray-400 group-hover:text-white" />
                </a>
                <a href={BUSINESS_INFO.facebookUrl} target="_blank" rel="noopener noreferrer" className="w-11 h-11 sm:w-12 sm:h-12 border border-white/10 flex items-center justify-center hover:bg-blue-600 hover:border-blue-600 transition-all group">
          ...
          <div className="pt-10 sm:pt-12 border-t border-white/5 flex flex-col md:flex-row justify-between items-center md:items-start gap-5 sm:gap-6">
            ...
            <div className="flex flex-wrap items-center justify-center md:justify-end gap-5 sm:gap-8">
              <Link to="/privacy" className="text-gray-600 hover:text-white text-[10px] uppercase tracking-[0.18em] sm:tracking-[0.2em] transition-colors">Політика конфіденційності</Link>
              <Link to="/terms" className="text-gray-600 hover:text-white text-[10px] uppercase tracking-[0.18em] sm:tracking-[0.2em] transition-colors">Умови сервісу</Link>
            </div>
          </div>
        </div>
      </footer>
